//                               page load                               //

window.addEventListener("DOMContentLoaded", async function () {
    const user = await requireLogin("Admin");

    if (!user) return;

    renderNavbar(user, "/Admin/Courses");

    await loadCourses();

    //          live search: re-fetch whenever the admin types          //

    document.getElementById("courseSearch").addEventListener("input", async function () {
        await loadCourses(this.value.trim());
    });
});

//                               load course list                               //

async function loadCourses(searchTerm) {
    // DATA FLOW: GET /api/courses?search=... -> { success, data: [ { ..., isActive } ] }
    let url = "/api/courses";

    if (searchTerm) {
        url += "?search=" + encodeURIComponent(searchTerm);
    }

    const result = await apiGet(url);

    const coursesList = document.getElementById("coursesList");

    if (!result.success || result.data.length === 0) {
        coursesList.innerHTML = `<p class="text-muted">No courses found.</p>`;
        return;
    }

    let cardsHtml = "";

    for (const course of result.data) {

        //          disabled courses get a badge + an Enable button (admin only)          //

        const statusBadge = course.isActive
            ? ""
            : `<span class="badge bg-secondary ms-2">Disabled</span>`;

        const toggleButton = course.isActive
            ? `<button class="btn btn-outline-danger btn-sm" onclick="disableCourse(${course.courseId})">Disable</button>`
            : `<button class="btn btn-success btn-sm" onclick="enableCourse(${course.courseId})">Enable</button>`;

        cardsHtml += `
            <div class="card border-glow-card mb-3 ${course.isActive ? "" : "opacity-75"}">
                <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div>
                        <h6 class="mb-1">${escapeHtml(course.title)}${statusBadge}</h6>
                        <small class="text-muted">By ${escapeHtml(course.instructorName)} \u2022 ${escapeHtml(course.category)} \u2022 ${escapeHtml(course.level)} \u2022 ${course.chapterCount} chapter(s)</small>
                    </div>
                    <div class="d-flex gap-2">
                        <a class="btn btn-outline-primary btn-sm" href="/ManageCourse?id=${course.courseId}">Manage / Edit</a>
                        ${toggleButton}
                    </div>
                </div>
            </div>`;
    }

    coursesList.innerHTML = cardsHtml;
}

//                               disable course (soft delete)                               //

async function disableCourse(courseId) {
    if (!(await showConfirm("Disable this course? Students will no longer see it, but nothing is deleted and you can re-enable it any time.", "Disable Course", "Disable", true))) return;

    // DATA FLOW: DELETE /api/courses/5 -> soft delete (IsActive = false)
    const result = await apiDelete("/api/courses/" + courseId);

    if (!result.success) {
        alert(result.message);
        return;
    }

    await loadCourses(document.getElementById("courseSearch").value.trim());
}

//                               enable course                               //

async function enableCourse(courseId) {
    // DATA FLOW: PUT /api/courses/5/status { isActive: true } -> re-enables the course
    const result = await apiPut("/api/courses/" + courseId + "/status", { isActive: true });

    if (!result.success) {
        alert(result.message);
        return;
    }

    await loadCourses(document.getElementById("courseSearch").value.trim());
}
