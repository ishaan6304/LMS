...
const FX_CONFIG = {
    ...
    lineWaves: {
        ...
        innerLineCount: 32,           // line density in the centre
        outerLineCount: 36,           // line density at the edges
        warpIntensity: 0.1,           // how much the lines bend
        rotation: -45,                // rotation in degrees
        edgeFadeWidth: 0,             // fade lines out near edges (0 = off)
        colorCycleSpeed: 5,           // how fast the colours shift around
    ...
    specularButton: {
        radius: 60                   // cursor distance (px) where the rim appears
    },
    ...
    dock: {
        range: 80,                   // distance (px) where icons start growing
        magnify: 0.40                 // 0.35 = icons grow up to 135%
    },

    //          GooeyNav navbar (react bits "Gooey Nav")          //
    gooeyNav: {
        particleCount: 0,            // particles per burst
        animationTime: 500,           // ms - base speed of pill + particles
        timeVariance: 200,            // ms - random extra time per particle
        particleDistances: [56, 8],   // [spread, settle] distance in px
        particleR: 0,                // rotation randomness of the burst
        navDelay: 0                 // ms to wait so the burst shows before the page changes
    }
};
...
//                               GooeyNav (react bit port)                               //

//          the navbar: the current page's link sits in a bright pill,          //
//          clicking another link fires a gooey particle burst, then          //
//          the browser follows the link. api.js calls initGooeyNav()          //
//          right after it renders the navbar html.          //

function initGooeyNav() {
    const cfg = FX_CONFIG.gooeyNav;
    ...
    if (!nav || !filterEl || !textEl) return;

    // small random +/- wobble, straight from the react bit
    ...
    function makeParticles(element) {
        const d = cfg.particleDistances;
        const r = cfg.particleR;
        const bubbleTime = cfg.animationTime * 2 + cfg.timeVariance;
        element.style.setProperty("--time", bubbleTime + "ms");

        for (let i = 0; i < cfg.particleCount; i++) {
            const t = cfg.animationTime * 2 + noise(cfg.timeVariance * 2);
            const start = getXY(d[0], cfg.particleCount - i, cfg.particleCount);
            const end = getXY(d[1] + noise(7), cfg.particleCount - i, cfg.particleCount);
            const rotate = noise(r / 10);
            const colorIndex = 1 + Math.floor(Math.random() * 4);
            ...
            particle.style.setProperty("--scale", String(1 + noise(0.2)));
            particle.style.setProperty("--color", "var(--color-" + colorIndex + ", white)");
            ...
            setTimeout(function () {
                try { element.removeChild(particle); } catch (err) { /* already gone */ }
            }, t);
        }
    ...
    // move the gooey blob + text flash layers over one nav item
    ...
    // on page load: park the pill on the current page's link
    ...
    // on click: burst the particles, slide the pill, then follow the link
    for (const li of items) {
        li.addEventListener("click", function (e) {
            const link = li.querySelector("a");
            if (!link || li.classList.contains("active")) return;
            e.preventDefault();
            ...
            setTimeout(function () { window.location.href = link.href; }, cfg.navDelay);
        });
    }
}
